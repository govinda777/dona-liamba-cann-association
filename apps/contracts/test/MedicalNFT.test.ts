...
describe("MedicalNFT", function () {
  ...
  beforeEach(async function () {
    ...
    const MedicalNFTFactory = (await ethers.getContractFactory(
      "MedicalNFT",
    )) as MedicalNFT__factory;
  ...
  describe("Minting", function () {
    it("Should allow a doctor to mint a prescription", async function () {
      ...
      await expect(
        medicalNFT
          .connect(doctor)
          .mintPrescription(patient.address, "QmHash", validityDays),
      )
  ...
  describe("Validation", function () {
    ...
    it("Should return false for expired prescription", async function () {
      ...
      await medicalNFT
        .connect(doctor)
        .mintPrescription(patient.address, "QmHash", 0);
  ...
  describe("Soulbound", function () {
    ...
    it("Should revert on transfer", async function () {
      await expect(
        medicalNFT
          .connect(patient)
          .transferFrom(patient.address, otherAccount.address, 0),
