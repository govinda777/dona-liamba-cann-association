...
export function LeadCaptureForm() {
  ...
  const generateGuidePDF = () => {
    ...
    doc.text('Como Iniciar Seu Tratamento Legal no Brasil', 20, 30);

    doc.setFontSize(12);
    const content = [
      '1. Consulta Medica: O primeiro passo e encontrar um medico prescritor.',
      '2. Prescricao: O medico emitira uma receita com os produtos indicados.',
      '3. Cadastro na Anvisa: Para importar, e necessario o cadastro no site da Anvisa.',
      '4. Associacoes: Voce pode se associar a uma entidade para acesso facilitado.',
      '',
      'Dona Liamba - Conectando saude e bem-estar.'
    ];
  ...
  if (status === 'success') {
    return (
      <div className="text-center p-6 bg-white/10 rounded-2xl backdrop-blur-md border border-white/20">
        ...
        <h4 className="text-xl font-bold text-white mb-2">Inscricao Realizada!</h4>
        <p className="text-white/80">Seu guia foi baixado e em breve voce recebera mais informacoes.</p>
        <Button
          variant="ghost"
  ...
  return (
    <form className="flex flex-col sm:flex-row gap-3 max-w-xl mx-auto" onSubmit={handleSubmit}>
      ...
      {status === 'error' && (
        <p className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-red-200 text-xs">
